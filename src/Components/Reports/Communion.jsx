            ...
            <thead>
              <tr>
                {tableHeading.map(heading => (
                  <th key={heading} className="px-4 py-2 text-base text-left text-gray-700 bg-white border-b dark:bg-gray-700 dark:text-gray-400">{heading}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {currentPageData.map((item, index) => (
                <tr key={index} className="px-2 py-2 text-left border-t border-gray-200">
                  ...
                  <td className="px-4 py-2 text-sm">{moment(item.communion_date).format('DD-MM-YYYY')}</td>
